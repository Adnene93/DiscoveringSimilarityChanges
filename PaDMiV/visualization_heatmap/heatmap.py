...
def generateHeatMap(dataset,destination,color='RdYlGn',vmin=None,vmax=None,organize=False,title=None,showvalues_text=True,only_heatmap=True):
    ...
    rower=[unicodedata.normalize('NFD', unicode(str(rower[k]),'iso-8859-1')).encode('ascii', 'ignore')[4:] for k in sorted(rower)]
    header=[unicodedata.normalize('NFD', unicode(str(header[k]),'iso-8859-1')).encode('ascii', 'ignore')[4:]  for k in sorted(header)]  
